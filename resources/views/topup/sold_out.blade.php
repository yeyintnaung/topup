                    ...
                    <div class="col s12">
                        <div class="card">                       
                            <div class="card-content">
                                ...
                                <span class="card-title">Sold Out Topups
                                    <span class="tag-about red">Total- 
                                        <?php $total = App\topup::where('status','=',0)->sum('amount');?>
                                        {{(!empty($total) ? number_format($total) : '0')}} Ks
                                    </span> 
                                    <span class="tag-about blue">MPT- 
                                        <?php $mpt = App\topup::where('status','=',0)->where('card','=',1)->sum('amount');?>
                                        {{(!empty($mpt) ? number_format($mpt) : '0')}} Ks
                                    </span>
                                    <span class="tag-about teal">
                                        Ooredoo- 
                                        <?php $ooredoo = App\topup::where('status','=',0)->where('card','=',2)->sum('amount');?>
                                        {{(!empty($ooredoo) ? number_format($ooredoo) : '0')}} Ks
                                    </span>
                                    <span class="tag-about green">
                                        Telenor- 
                                        <?php $telenor = App\topup::where('status','=',0)->where('card','=',3)->sum('amount'); ?>
                                        {{(!empty($telenor) ? number_format($telenor) : '0')}} Ks
                                    </span>
                                    <span class="tag-about indigo">
                                        Telenor[Magic]- 
                                        <?php $telenor2 = App\topup::where('status','=',0)->where('card','=',4)->sum('amount'); ?>
